...
class Excel2JsonRes(val excel: String, val res: String) {
...
//                println("..." + "[" + firstRow.firstCellNum + "," + firstRow.lastCellNum + "]")
    ...
    private fun parse_(sheet: XSSFSheet) {
        ...
        for (languageIndex in (languageRow.firstCellNum + 1)..languageRow.lastCellNum) {
            ...
            val jsonFile = File(dir, "${sheet.sheetName}.json")
            if (!jsonFile.exists()) {
                jsonFile.createNewFile()
            }
            FileOutputStream(jsonFile).bufferedWriter().use { writer -> writer.write(jsonObject.toString()) }
